...
const AdminControlPanel = () => {
  ...
  const addInput = async () => {
    ...
    await axios.post('http://localhost:8000/api/inputs', { label: newInputLabel, visible: true });
    setNewInputLabel('');
    fetchInputs();
  };

  const toggleVisibility = async (id, currentVisibility) => {
    await axios.put(`http://localhost:8000/api/inputs/${id}`, { visible: !currentVisibility });
    fetchInputs();
  };

  const deleteInput = async (id) => {
    await axios.delete(`http://localhost:8000/api/inputs/${id}`);
    fetchInputs();
  };
  ...
  return (
    <div style={{
      ...
      <h2 style={{ textAlign: 'center', color: '#333' }}>Admin Input Control Panel</h2>
